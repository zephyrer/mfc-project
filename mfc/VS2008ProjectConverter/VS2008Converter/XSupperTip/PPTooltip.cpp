...
BOOL CPPToolTip::OnSetCursor(CWnd* /*pWnd*/, UINT /*nHitTest*/, UINT /*message*/) 
{
	...
	{
		TRACE(_T("cursor over hyperlink\n"));
...
BOOL CPPToolTip::RelayEvent(MSG* pMsg)
{
	...
	if (m_nTooltipState == PPTOOLTIP_STATE_SHOWN)
	{
		...
		{
			TRACE(_T("CPPToolTip::RelayEvent:   WM_KEYDOWN or WM_KEYUP\n"));
			if (pMsg->wParam == m_tiNextTool.nVirtualKeyCode)
			{
				TRACE(_T("CPPToolTip::RelayEvent:  virtual key code seen\n"));
				if (pMsg->message == WM_KEYDOWN)
				{
					if (::IsWindow(m_hParentWnd))
					{
						TRACE(_T("sending message to parent\n"));
					...
					{
						TRACE(_T("ERROR: parent window doesn't exist\n"));
					}
				}
		...
		if ((pMsg->message == 0x4d) && (m_tiNextTool.nVirtualKeyCode == VK_F1))
		{
			...
			TRACE(_T("CPPToolTip::RelayEvent:  MFC magic help message\n"));
			return TRUE;
		}
	}
			...
			else
			{
				...
				else 
				{
					...
					else
					{
						...
						{
							TRACE(_T("calling SetAutoPopTimer ==========\n"));
...
void CPPToolTip::PrepareDisplayTooltip(LPPOINT lpPoint)
{
	...
	else
	{
		...
		TRACE(_T("rMonitor:  left=%d  top=%d  right=%d  bottom=%d --------\n"),
...
{
	TRACE(_T("in CPPToolTip::GetWndFromPoint:  m_hParentWnd=0x%X\n"), m_hParentWnd);
	...
	if (m_bIgnoreEnabledStatus)		//+++hd
	{
		...
		TRACE(_T("ChildWindowFromPoint returned 0x%X\n"), hWnd);
	}
	else
	{
		hWnd = ::WindowFromPoint(pt);
		TRACE(_T("WindowFromPoint returned 0x%X\n"), hWnd);
	}
	if (NULL != hWnd)
	{
		// try to hit combobox instead of edit control for CBS_DROPDOWN styles
		HWND hWndTemp = ::GetParent(hWnd);
		TRACE(_T("GetParent returned 0x%X\n"), hWndTemp);
		if (NULL != hWndTemp)
		{
			if (!IsComboBoxControl(hWndTemp, CBS_DROPDOWN))
			{
				...
				if (NULL == hWndTemp)
				{
					TRACE(_T("ChildWindowFromPoint returned NULL\n"));
					return NULL;
				}
